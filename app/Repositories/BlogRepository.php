...
class BlogRepository extends BaseRepository {
     ...
     */
    private function savePost($post, $inputs, $user_id = null, $path)
    {
        $post->title = $inputs['title'];
        $post->summary = '<img alt="" src="'.$path.'" style="float:left; height:128px; width:128px" />';
    ...
    {
        return $this->model
            ->select('id', 'created_at', 'updated_at', 'title', 'slug', 'user_id', 'summary','content')
                        ->whereActive(true)
                        ->with('user')
                        ->latest();
    }
     ...
     * @return Illuminate\Support\Collection
     */
    public function indexMyFront($n, $id)
    {
        $query = $this->queryActiveWithUserOrderByDate();
     ...
     * @return void
     */
    public function store($inputs, $user_id, $path)
    {
        $post = $this->savePost(new $this->model, $inputs, $user_id, $path);
